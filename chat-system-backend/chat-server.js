...
const checkRateLimit = (deviceId, type) => {
  ...
  if (type === 'session') {
    const limit = rateLimits.sessions.get(deviceId) || { count: 0, resetTime: now + 3600000 };
    if (now > limit.resetTime) {
      limit.count = 0;
      limit.resetTime = now + 3600000;
    }
    if (limit.count >= RATE_LIMITS.MAX_SESSIONS_PER_HOUR) {
      return { allowed: false, reason: 'Too many sessions. Please try again later.' };
    }
    limit.count++;
  ...
  if (type === 'message') {
    const limit = rateLimits.messages.get(deviceId) || {
      ...
      hourCount: 0, hourResetTime: now + 3600000
    };
...
const HUMAN_SUPPORT_KEYWORDS = [
  'human', 'real person', 'agent', 'operator', 'representative',
  'gerçek insan', 'gerçek kişi', 'destek', 'müşteri temsilcisi',
  'insan', 'canlı destek', 'temsilci', 'yetkili',
  'talk to human', 'speak to agent', 'real support'
];

// AI Response templates based on keywords
const AI_RESPONSES = {
  greeting: {
    keywords: ['hello', 'hi', 'hey', 'salam', 'merhaba', 'good morning', 'good afternoon', 'good evening'],
    responses: [
  ...
  pricing: {
    keywords: ['price', 'cost', 'pricing', 'how much', 'fee', 'qiymət', 'fiyat', 'ücret', 'ne kadar'],
    responses: [
  ...
  hours: {
    keywords: ['hours', 'open', 'available', 'when', 'working hours', 'saat', 'açık', 'çalışma saatleri'],
    responses: [
  ...
  help: {
    keywords: ['help', 'support', 'assist', 'problem', 'issue', 'yardım', 'sorun', 'problem'],
    responses: [
...
function wantsHumanSupport(message) {
  ...
  return HUMAN_SUPPORT_KEYWORDS.some(keyword => lowerMessage.includes(keyword.toLowerCase()));
}
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (message) => {
    try {
      ...
      if (data.type === 'register') {
        ...
        chatClients.set(clientSessionId, { ws, type: clientType, project: currentPage, ip: clientIP });
        console.log(`📱 Chat client registered: ${clientType} (${clientSessionId}) on ${currentPage} from ${clientIP}`);
        ...
        ws.send(JSON.stringify({ type: 'registered', sessionId: clientSessionId }));
      ...
      if (data.type === 'agent_register') {
        ...
        try {
          ...
          chatClients.forEach((client) => {
            if (client.type === 'user' && client.ws.readyState === WebSocket.OPEN) {
      ...
      if (data.type === 'agent_status_change') {
        ...
        if (activeAgents.has(agentId)) {
          ...
          if ((status === 'away' || status === 'busy') && previousStatus === 'online') {
            console.log(`📤 Agent ${agentId} is now ${status}, removing session assignments`);
            // Remove all sessions assigned to this agent
            for (const [sessionId, assignedAgent] of sessionAssignments.entries()) {
              if (assignedAgent === agentId) {
                sessionAssignments.delete(sessionId);
                console.log(`🔓 Released session ${sessionId} from agent ${agentId}`);
              }
            }
          }
      ...
      if (data.type === 'message_delivered') {
        messageStatuses.set(data.messageId, { status: 'delivered', timestamp: new Date() });
      ...
      if (data.type === 'message_read') {
        messageStatuses.set(data.messageId, { status: 'read', timestamp: new Date() });
      ...
      if (data.type === 'mark_all_read') {
        if (clientType === 'user') {
          ...
          console.log(`✅ Agent marked messages as read for session: ${data.targetSessionId}`);
          console.log(`🔍 All connected clients:`, Array.from(chatClients.keys()));
          const targetClient = chatClients.get(data.targetSessionId);
          console.log(`🔍 Target client found:`, targetClient ? 'YES' : 'NO');
          if (targetClient) {
            console.log(`🔍 WebSocket state:`, targetClient.ws.readyState, '(1=OPEN)');
          }
          if (targetClient && targetClient.ws.readyState === WebSocket.OPEN) {
            console.log(`📤 Sending 'all_messages_read_by_agent' to user ${data.targetSessionId}`);
            targetClient.ws.send(JSON.stringify({
              type: 'all_messages_read_by_agent',
              sessionId: data.targetSessionId
            }));
          } else {
            console.log(`⚠️  User ${data.targetSessionId} not connected or not ready`);
          }
      ...
      if (data.type === 'user_message') {
        ...
        try {
          await pool.query(
            ...
            [clientSessionId, 'Customer', data.text, 'new', project, data.messageId, 'customer', data.fileUrl || null, data.fileType || null, data.fileName || null]
          );
          console.log(`💾 Saved customer message to database: ${data.messageId}`);
        } catch (dbError) {
        ...
        console.log(`📋 Session ${clientSessionId} - assignedAgentId: ${assignedAgentId}, wantsHuman: ${wantsHuman}`);
        console.log(`📋 All session assignments:`, Object.fromEntries(sessionAssignments));
        ...
        if (activeAgents.size > 0) {
          ...
          if (ws.readyState === WebSocket.OPEN) {
            console.log(`📬 Sending 'delivered' status for message ${data.messageId} to user`);
            ws.send(JSON.stringify({
              type: 'message_status_update',
              messageId: data.messageId,
              status: 'delivered'
      ...
      if (data.type === 'admin_reply') {
        console.log(`📨 Admin reply to session ${data.targetSessionId}: ${data.text}`);
        const targetClient = chatClients.get(data.targetSessionId);
        if (targetClient && targetClient.ws.readyState === WebSocket.OPEN) {
          targetClient.ws.send(JSON.stringify({
            type: 'admin_message',
      ...
      if (data.type === 'agent_message') {
        ...
        console.log(`📨 Agent ${agentId} sending message to session ${targetSessionId}`);
        ...
        try {
          await pool.query(
            ...
            [targetSessionId, data.agentName || 'Agent', data.text, 'replied', null, data.messageId, 'agent', agentId, data.agentName, data.fileUrl || null, data.fileType || null, data.fileName || null]
          );
        ...
        if (!assignedAgentId && agentStatus === 'online') {
          console.log(`📋 Auto-assigning session ${targetSessionId} to online agent ${agentId}`);
          assignSessionToAgent(targetSessionId, agentId);
        } else if (agentStatus !== 'online') {
          console.log(`ℹ️ Agent ${agentId} is ${agentStatus}, not assigning session (AI will continue)`);
        }
        ...
        if (customerClient && customerClient.ws.readyState === WebSocket.OPEN) {
          console.log(`✅ Forwarding agent message to customer ${targetSessionId}`);
          ...
          console.log(`📴 Customer ${targetSessionId} is offline or connection closed`);
        }
      ...
      if (data.type === 'file_deleted') {
        ...
        console.log(`🗑️ File deleted from session ${targetSessionId}, message ${messageId}`);
        ...
        if (clientType === 'user') {
          ...
          if (assignedAgentId) {
            ...
            if (agent && agent.ws && agent.ws.readyState === WebSocket.OPEN) {
              ...
              console.log(`✅ Notified agent ${assignedAgentId} about file deletion`);
            }
          }
        }
        ...
        if (clientType === 'agent') {
          ...
          if (customerClient && customerClient.ws.readyState === WebSocket.OPEN) {
            ...
            console.log(`✅ Notified customer ${targetSessionId} about file deletion`);
          }
          ...
          broadcastToAgents({
            ...
            sessionId: targetSessionId,
            messageId: messageId
          }, clientSessionId); // Exclude sender
        }
      }
    } catch (error) {
  ...
  ws.on('close', () => {
    if (clientSessionId) {
      ...
      console.log(`📴 Chat client disconnected: ${clientType} (${clientSessionId})`);
      ...
      if (clientType === 'agent') {
        ...
        if (agent) {
          ...
          console.log(`👤 Agent disconnected: ${agent.name} (${clientSessionId})`);
...
app.get('/api/agents', async (req, res) => {
  try {
    const result = await pool.query('SELECT id, username, name, email, role, created_at FROM agents ORDER BY created_at');

    // Merge with online status
    const agents = result.rows.map(agent => {
      const onlineAgent = Array.from(activeAgents.values()).find(a => a.id === agent.id);
      return {
        ...agent,
...
app.delete('/api/agent/:agentId', async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT id FROM agents WHERE id = $1', [agentId]);
...
app.delete('/api/chat/block/:sessionId', async (req, res) => {
  try {
    ...
    await pool.query('DELETE FROM chat_blocklist WHERE session_id = $1', [sessionId]);
...
app.get('/api/chat/blocklist', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM chat_blocklist ORDER BY created_at DESC');
    res.json({
      success: true,
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const host = req.get('x-forwarded-host') || req.get('host') || 'localhost:5003';
...
app.post('/api/contact', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [name, name, message, 'new', project || null, fileUrl || null, fileType || null, fileName, 'customer']
    );
...
app.post('/api/contact/reply', async (req, res) => {
  try {
    ...
    console.log(`💾 Reply saved to database for user ${userId}, WebSocket delivery handled separately`);
...
app.delete('/api/contact/user/:userId', async (req, res) => {
  try {
    ...
    for (const msg of messagesResult.rows) {
      await pool.query('DELETE FROM contact_replies WHERE message_id = $1', [msg.id]);
    }
...
app.post('/api/chat/customer', async (req, res) => {
  try {
    const { sessionId, fullName, email, phone, clientIp, clientCountry } = req.body;
    ...
    if (ipAddress !== 'Unknown' && !ipAddress.startsWith('127.') && !ipAddress.startsWith('192.168.')) {
      try {
        const vpnCheckResponse = await fetch(`http://proxycheck.io/v2/${ipAddress}?vpn=1&asn=1`);
        const vpnData = await vpnCheckResponse.json();

        if (vpnData[ipAddress]) {
          isVpn = vpnData[ipAddress].proxy === 'yes' || vpnData[ipAddress].type === 'VPN';
          vpnProvider = vpnData[ipAddress].provider || null;
          console.log(`🔍 VPN Check for ${ipAddress}: ${isVpn ? 'VPN/Proxy detected' : 'Clean'}`);
        }
      } catch (vpnError) {
    ...
    if (existingCustomer.rows.length > 0) {
      ...
      const result = await pool.query(
        ...
        [fullName, email, phone, ipAddress, country, isVpn, vpnProvider, sessionId]
      );
      ...
      const result = await pool.query(
        ...
        [sessionId, fullName, email, phone, ipAddress, country, isVpn, vpnProvider]
      );
      customerId = result.rows[0].customer_id;
    }
    ...
    console.log(`📋 Customer info saved for session ${sessionId}: ${fullName} (${country})`);
...
app.post('/api/chat/customer/:customerId/flag', async (req, res) => {
  try {
    ...
    if (isScam) {
      const customer = await pool.query('SELECT session_id FROM chat_customers WHERE customer_id = $1', [customerId]);
